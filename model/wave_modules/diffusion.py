...
class DWSR(nn.Module):
    def __init__(self, in_channels, features, out_channels, kernel_size=3, padding=1, stride=1, groups=1, depth=10):
        ...
        self.conv_layers = [
            nn.Conv2d(in_channels, features, kernel_size=kernel_size, padding=padding,
                          stride=stride, bias=False, groups=groups),
            nn.ReLU()
        ]
        for i in range(depth):
            self.conv_layers.append(nn.Conv2d(features, features, kernel_size=kernel_size, padding=padding,
                          stride=stride, bias=False, groups=groups))
            self.conv_layers.append(nn.ReLU())
        self.conv_layers.append(
            nn.Conv2d(features, out_channels, kernel_size=kernel_size, padding=padding,
                      stride=stride, bias=False, groups=groups)
        )
...
class GaussianDiffusion(nn.Module):
    ...
    def p_losses(self, x_in, noise=None):
        ...
        x_noisy = self.q_sample(
            x_start=x_start-x_sr, continuous_sqrt_alpha_cumprod=continuous_sqrt_alpha_cumprod.view(-1, 1, 1, 1), noise=noise)

        if not self.conditional:
            x_recon = self.denoise_fn(x_noisy, continuous_sqrt_alpha_cumprod)
        else:
            x_recon = self.denoise_fn(
                torch.cat([x_lr, x_noisy], dim=1), continuous_sqrt_alpha_cumprod)
